          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              {filteredFacilities.map(f => (
                <div key={f.id} className="stitch-border p-4 hover:border-gray-400 transition-colors bg-white cursor-pointer">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-sm font-semibold text-gray-900 leading-tight">{f.name}</h3>
                    <span className={`h-2 w-2 shrink-0 rounded-full ${getLoadColor(f.load)} mt-1`}></span>
                  </div>
            ...
            {filteredFacilities.map((f) => (
              ...
              >
                <div className="relative flex flex-col items-center">
                  <div className={`h-5 w-5 rounded-full border-4 border-white stitch-shadow z-10 ${getLoadColor(f.load)} group-hover:scale-110 transition-transform`}></div>
